...
Transform.prototype.setRepeat = function(cb, el, active, state) {
	...
	if(transform.repeat) {
		return function() {
			...
			if(transform.repeat.repeated <= 0="" 0)="" delete="" transform.repeat.repeated;="" else="" if(!isnan(transform.repeat.repeated))="" {="" settimeout(function()="" transform.animation(cb,="" el,="" active,="" state);="" if(!transform.repeat.loop="" ||="" transform.repeat.loop="" &&="" transform.repeat.loopduration)="" transform.repeat.repeated--;="" },="" transform.repeat.delay="" 0);="" }="" };="" return="" null;="" transform.prototype.animatetransform="function(transitionTime," cb,="" state,="" repeat,="" snapelem)="" var="" transform="this;" snapelem.animate({="" transform:="" transform.transformstring="" transitiontime,="" this.easing,="" function()="" if(transform.callbacks.length="==" cb)="" cb();="" transform.callbacks.foreach(function(callback)="" callback(function()="" transform.waitingfor--;="" if(transform.waitingfor="==" if(repeat)="" repeat();="" if(cb)="" });="" this;="" transform.prototype.animatepath="function(path," if(path="" !="=" undefined)="" d:="" path="" this.easing);="" transform.prototype.animatepoints="function(points," if(points="" currentpoints="snapElem.attr('points')," currentpointsarr="[];" for(var="" i="0;" <="" currentpoints.length;="" i++)="" point="parseInt(currentPoints[i]);" if(!isnan(point))="" currentpointsarray.push(point);="" snapelem.animate(currentpointsarr,="" points,="" function(val)="" snapelem.attr({="" points:val="" transform.prototype.animatedrawpath="function(path," as="" a="" percentage="" that="" needs="" to="" be="" drawn="" linelength="snapElem.getTotalLength();" 'stroke-dasharray':="" +="" '="" if="" stroke-dashoffset="" is="" pre="" set="" in="" the="" svg="" and="" greater="" than="" actual="" length="" offset="parseInt(snapElem.attr('stroke-dashoffset'));" if(offset=""> lineLength) snapElem.attr({ 'stroke-dashoffset': lineLength });
